...
export class JobPreferencesComponent implements OnInit {
    ...
    createOrModifyCommonProfileDraft() {
        ...
        if (this.jobPreferenceForm.valid) {
            ...
            if (this.tempList.length == 0) {
                ...
                const mergedArray: JobPreference[] = this.tempList.concat(this.tableList);
        ...
        this.draftService.findDraftByIdCandidate(idCandidate).subscribe(
            (data) => {
                ...
                if (data.data != null) {
                    ...
                    this.commonProfileDraft.jobPreferences.forEach((jobPreference: JobPreference) => {
                        ...
                        const selectedJobCategory = this.jobCategories.find(
                            (jobCategory: JobCategory) => jobCategory.idJobCategory == jobPreference.idJobCategory);
    ...
    jobPreferenceTempListTable() {
        ...
        if (this.jobPreferenceForm.valid) {
            ...
            const selectedJobCategory = this.jobCategories.find(
                (jobCategory: JobCategory) => jobCategory.idJobCategory == formValue.idJobCategory);
